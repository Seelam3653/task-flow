...
// ---------------------------------------------------------------------------
// MAIN NAVIGATION WRAPPER WITH HEADER & BOTTOM NAV
// ---------------------------------------------------------------------------
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  void _loadData() {
    final rawTasks = widget.prefs.getString(_tasksKey);
    if (rawTasks != null && rawTasks.isNotEmpty) {
      try {
        final List<dynamic> list = jsonDecode(rawTasks);
        _tasks = list.map((e) => TaskItem.fromJson(e)).toList();
      } catch (_) {
        _tasks = _seedTasks();
      }
    } else {
    ...
    final rawHabits = widget.prefs.getString(_habitsKey);
    if (rawHabits != null && rawHabits.isNotEmpty) {
      try {
        final List<dynamic> list = jsonDecode(rawHabits);
        _habits = list.map((e) => HabitItem.fromJson(e)).toList();
      } catch (_) {
        _habits = _seedHabits();
      }
    } else {
  ...
  void _onTasksUpdated() {
    setState(() {});
    widget.prefs.setString(_tasksKey, jsonEncode(_tasks.map((t) => t.toJson()).toList()));
  }

  void _onHabitsUpdated() {
    setState(() {});
    widget.prefs.setString(_habitsKey, jsonEncode(_habits.map((h) => h.toJson()).toList()));
  }
